=============== Change Logging ==================
--------------- transmit v2.9 -----------------
1.报警历史表alarmhistorysearchtable增加 IsSuccess 字段 否则导致 历史报警导表错误
2.监测中心四期新协议 (马赛克轮播,前端停止,小卡卡号查询,视频停止,前端属性配置)
3.报警门限 开关 下发时候 等待8秒 并且只给 一对一板卡的一个通道下发
4.自动录像记录表channelremapping表增加IpmIndex 字段
5.增加并修改自动录像下发时 跟tsc和ipm 节目 与通道 分配问题
6.自动录像删除 修改 给板卡 和 ipm 下发的index 号
7.修改录像路数查询 返回算法  一个高清节目按照 5个标清来算
8.门限 给板卡下发时候 BER 为科学记数法表示 问题
9.增加定时重启 轮播板卡任务
10.增加 不规则频点自动扫描
